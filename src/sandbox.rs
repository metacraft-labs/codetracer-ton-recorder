...
// this constant so the sandbox surface no longer carries a `format`
...
pub fn trace_sandbox(vm_log_path: &Path, source_path: &Path, out_dir: &Path) -> Result<()> {
    ...
    // -- Set up the trace writer (CTFS only) --
